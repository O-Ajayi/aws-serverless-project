...
resource "aws_cloudwatch_log_group" "s3_push-event-consumer" {
  name = "s3_push-event-consumer"
  tags = {
    "environment" = "dev"
    "Name"        = "s3_push-event-consumer"
    "project"     = "s3_push"
...
resource "aws_ecr_repository" "s3_push" {
  name                 = "s3_push"
...
resource "aws_ecs_task_definition" "s3_push" {
  family                   = "s3_push"
  ...
  container_definitions = jsonencode([
    {
      name              = "s3_push"
      image             = "${aws_ecr_repository.s3_push.repository_url}:latest"
      memoryReservation = 128
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          awslogs-group         = "${aws_cloudwatch_log_group.s3_push-event-consumer.id}"
