...
resource "aws_lambda_function" "experiment_lambda" {
  ...
  s3_bucket = aws_s3_bucket.experiment_bucket.id #resource.aws_s3_bucket.experiments_bucket.id
...
# This would be the bucket for the lambda application package
resource "aws_s3_bucket" "experiment_bucket" {
  bucket = "resiliencyvr-package-build-bucket-apps"
}
